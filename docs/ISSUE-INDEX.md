...
- `pilot-reply` now lets Codex route the learner's current answer to the next baseline, daily, or final pilot slot without asking the learner for phase/card/day internals, then refreshes the next learner-facing card artifact and returns a learner-safe coaching summary.
- `scripts/phase15-owner-pilot-reply-routing-smoke.mjs` verifies baseline, daily, and final routing through `pilot-capture`, cockpit refresh, next-card refresh, learner-facing summary fields, and no learner-facing command-token leakage.

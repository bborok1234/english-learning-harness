...
| #262 | AIOS-38: Return pending learner prompt after no-save pilot intake | continue |
...
- Active learner cockpit state/html now links `artifacts/pilot/pilot-turn-packet.json/html` as a learner-safe Codex continuation surface when the local packet exists.
- `scripts/personal-learner-cockpit-pilot-turn-link-smoke.mjs` verifies turn-packet linkage, browser-rendered cockpit visibility, no extra answer save, preserved launch/next/reply links, and no command/issue/PR/rubric/audit leakage.
- `docs/phase-15-evidence/AIOS-32-cockpit-pilot-turn-link.md` records the active cockpit turn-link evidence boundary.
- `pilot-evidence-gap` now writes redacted JSON/HTML with required, collected, and remaining counts for Day 0 baseline, daily sessions, final sample, friction notes, local report, and direction decision.
- `scripts/phase15-owner-pilot-evidence-gap-smoke.mjs` verifies fresh/partial fixture gaps, rendered HTML, next safe continuation prompt, local links, no answer save, and no transcript/friction/internal-language leakage.
- `docs/phase-15-evidence/AIOS-33-pilot-evidence-gap.md` records the redacted evidence-gap evidence boundary.
...
- `pilot-intake` now returns `learnerFacing.resumePrompt`, `learnerFacing.quickReplies`, and preview `resume` content sourced from the current `pilot-next` card so Codex can resume after no-save turns.
- `scripts/phase15-owner-pilot-intake-preview-smoke.mjs` verifies quick/meta/internal/direct previews include resume content, render it in HTML, and keep no-save invariants.
- `docs/phase-15-evidence/AIOS-38-intake-resume-prompt.md` records the no-save resume prompt evidence boundary.

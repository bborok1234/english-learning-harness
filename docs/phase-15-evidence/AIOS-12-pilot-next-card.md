...
- `pilot-next` now returns `quickReplies`, 2-3 copyable one-sentence English answer candidates, and the HTML renders them in a dedicated section.
...
The next-card smoke verifies baseline and daily next-card generation, learner-ready `assistantPrompt.text`, quick replies, rendered assistant prompt and quick-reply sections in HTML, cockpit linkage, progress advance after one fixture day, learner-facing privacy copy, and no leakage of internal pilot commands or PR/issue language into learner HTML or the prompt Codex should say.

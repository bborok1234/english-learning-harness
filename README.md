...
처음부터 평가 항목을 채우는 방식이 아닙니다. Codex가 작은 상황을 하나씩 주고, 학습자는 영어로 한 문장만 답합니다.

```text
이 하네스로 7일 영어 speaking pilot을 시작하자.
시험처럼 하지 말고, 3분 영어 스냅샷처럼 진행해줘.
한 번에 한 장면만 주고, 내가 영어로 한 문장 답하면 다음 장면으로 넘어가.
내 답변 원문은 로컬에만 저장하고 공개 이슈/PR에는 올리지 마.
```

Pilot prompt 원칙과 Day 0 미션 카드는 [docs/PILOT-PROMPTS.md](docs/PILOT-PROMPTS.md)에 정리되어 있습니다.
...
- 첫 연습은 프로젝트/명령어 설명이 아니라 일상 장면의 한 문장 대화로 시작합니다.
- 세션 증거를 로컬에 저장합니다.
...
node scripts/english-learning-harness.mjs scene --json
